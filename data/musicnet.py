...
class MusicNet(Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        if ctx_offset < 0:
            data, _ = sf.read(wav_file, start=offset, frames=chunk_frames, dtype='float32', always_2d=True)
            ctx = np.zeros_like(data)
        else:
            ctx, _ = sf.read(wav_file, start=ctx_offset, frames=chunk_frames * 2, dtype='float32', always_2d=True)
